...
import { ArrowLeft, Send, Paperclip, X, Save, Briefcase } from "lucide-react";
...
const formSchema = z.object({
  ...
  department: z.string().min(2, "Please enter your department/unit").max(100),
...
const FORM_STORAGE_KEY = "staffTicket_draft";
...
const StaffTicket = () => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      // Create ticket in Convex — staff ID goes into matric_number field for backend compatibility
      const result = await createTicket({
        matric_number: values.staffId,
        name: values.name,
        email: values.email,
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <Briefcase className="h-6 w-6 text-primary-foreground/80" />
                  <CardTitle className="text-xl md:text-3xl font-bold">
                    Create Support Ticket – Staff
                  </CardTitle>
                </div>
                <CardDescription className="text-primary-foreground/90 text-sm md:text-base mt-1">
                  Use your RUN Staff ID and RUN email address. All fields marked
                  with * are required.
                </CardDescription>
              </div>
              {lastSaved && (
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            type="email"
                            placeholder="e.g., jdoe@run.edu.ng"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          document.getElementById("staff-file-upload")?.click()
                        }
