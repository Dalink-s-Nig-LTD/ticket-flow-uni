...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (userRole) {
      // Only super admins are allowed to view the main dashboard
      if (userRole.role === "super_admin") return;

      // If department admin, send them to their department admin page (prefer role departments)
      if (userRole.role === "department_admin") {
        const deps = userRole.departments as string[] | null | undefined;
        if (deps && deps.length > 0) {
          const slug = encodeURIComponent(deps[0].replace(/\s+/g, "-"));
          navigate(`/admin/department/${slug}`);
          return;
        }
      }

      // Fallback: check server-side mapping for this session's email
      if (
  ...
  }, [hasChecked, sessionId, userRole, departmentsForSession, navigate]);
  ...
  const stats = {
    ...
    pending: filteredAndSortedTickets.filter(
      (t) => t.status.toLowerCase() === "pending"
    ).length,
    inProgress: filteredAndSortedTickets.filter(
      (t) => t.status.toLowerCase() === "in progress"
    ).length,
    resolved: filteredAndSortedTickets.filter(
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <Select value={natureFilter} onValueChange={setNatureFilter}>
                    ...
                    <SelectContent className="bg-popover z-50">
                      ...
                      <SelectItem value="Payment/Bursary">
                        Payment/Bursary
                      </SelectItem>
                      <SelectItem value="Exams/Results">
                        Exams/Results
                      </SelectItem>
                      <SelectItem value="Hostel/Accommodation">
